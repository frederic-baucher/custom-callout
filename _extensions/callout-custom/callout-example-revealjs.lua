...
function Div(el)
  ...
  if quarto.doc.isFormat("html") then
    if el.classes:includes('callout-example') then
	  quarto.log.output ( 'callout-example.lua - revealjs' )
	  ...
	  -- local walk = pandoc.walk_block(el, {
	    -- Header = function(subel)
		  ...
		  -- quarto.log.output ( 'callout-example.lua - revealjs: ' .. str_header )
		  -- -- quarto.log.output ( subel.content )
		  -- -- local walk = pandoc.walk_block(subel, {
		  -- -- Str = function(strel)
		  -- --  quarto.log.output ( 'callout-example.lua - revealjs - strel : ' )
	  ...
	  if ( div_attr ) then
		...
		if ( first_char == '{' ) and ( last_char == '}' ) then
			quarto.log.output ( 'callout-example.lua - revealjs' .. refactored_str_title )		
		else
	    ...
	    quarto.log.output ( 'callout-example.lua - revealjs - div_attr is nil' )
	  end
